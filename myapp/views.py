...
def Register_page(request):
    ...
    context = {
        ...
        'state': State.objects.filter(country_id=countryid) if countryid else None,
        'city': City.objects.filter(state_id=stateid) if stateid else None,
    }
...
def Edit_student_data(request):
    if request.method == "POST":
        ...
        'country': Country.objects.all(),
        'state': State.objects.filter(country_id=countryid) if countryid else None,
        'city': City.objects.filter(state_id=stateid) if stateid else None, "key2": user
        }
...
#     return render(request, 'newstate.html', context)
